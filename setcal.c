...
void is_function(Rel_t* rel_p, Universe_t* uni_p);
                ...
                if (line_count != 1) {
                    ...
                    if (strcmp("reflexive", commands[command_count - 1].command_string) == 0) {
                        if (command_arg_check(&commands[command_count - 1], 1) == 0) 
                            ...
                            if (rel_index_check(commands[command_count - 1], rels_array, rel_count, 0) != -1) 
                                is_reflexive(&rels_array[rel_index_check(commands[command_count - 1], rels_array, rel_count, 0)], &uni_array);
                            else return EXIT_FAILURE;
                        }
                    }
                    ...
                    if (strcmp("symmetric", commands[command_count - 1].command_string) == 0) {
                        if (command_arg_check(&commands[command_count - 1], 1) == 0) {
                            ...
                            if (rel_index_check(commands[command_count - 1], rels_array, rel_count, 0) != -1) {
                                is_symmetric(&rels_array[rel_index_check(commands[command_count - 1], rels_array, rel_count, 0)], &uni_array);
                            }
                            else return EXIT_FAILURE;
                    ...
                    if (strcmp("antisymmetric", commands[command_count - 1].command_string) == 0) {
                        if (command_arg_check(&commands[command_count - 1], 1) == 0) 
                            ...
                            if (rel_index_check(commands[command_count - 1], rels_array, rel_count, 0) != -1) 
                                is_antisymmetric(&rels_array[rel_index_check(commands[command_count - 1], rels_array, rel_count, 0)], &uni_array);
                            else return EXIT_FAILURE;
                        }
                    }  
                    ...
                    if (strcmp("transitive", commands[command_count - 1].command_string) == 0) {
                        if (command_arg_check(&commands[command_count - 1], 1) == 0) 
                            ...
                            if (rel_index_check(commands[command_count - 1], rels_array, rel_count, 0) != -1) 
                                is_transitive(&rels_array[rel_index_check(commands[command_count - 1], rels_array, rel_count, 0)], &uni_array);
                            else return EXIT_FAILURE;
                        }
                    }                    
                    ...
                    if (strcmp("domain", commands[command_count - 1].command_string) == 0) {
                        if (command_arg_check(&commands[command_count - 1], 1) == 0)
                            ...
                            if (rel_index_check(commands[command_count - 1], rels_array, rel_count, 0) != -1) {
                                rel_domain(&rels_array[rel_index_check(commands[command_count - 1], rels_array, rel_count, 0)]);
                            }
                            else return EXIT_FAILURE;
                        }
                    }
                    ...
                    if (strcmp("codomain", commands[command_count -1].command_string) == 0) {
                        if (command_arg_check(&commands[command_count - 1], 1) == 0)
                            ...
                            if (rel_index_check(commands[command_count - 1], rels_array, rel_count, 0) != -1) 
                                rel_codomain(&rels_array[rel_index_check(commands[command_count - 1], rels_array, rel_count, 0)]);
                            else return EXIT_FAILURE;
                        }
                    }
                    ...
                    if (strcmp("injective", commands[command_count -1].command_string) == 0) {
                        if (command_arg_check(&commands[command_count -1], 1) == 0)
                            ...
                            if (rel_index_check(commands[command_count - 1], rels_array, rel_count, 0) != -1) {
                                if (rel_inject(&rels_array[rel_index_check(commands[command_count - 1], rels_array, rel_count, 0)], uni_array) != 0) 
                                    printf("true\n");
                                else printf("false\n");
                            }    
                            else return EXIT_FAILURE;
                    ...
                    if (strcmp("surjective", commands[command_count -1].command_string) == 0) {
                        if (command_arg_check(&commands[command_count -1], 1) == 0)
                            ...
                            if (rel_index_check(commands[command_count - 1], rels_array, rel_count, 0) != -1) {
                                if (rel_surject(&rels_array[rel_index_check(commands[command_count - 1], rels_array, rel_count, 0)], uni_array) != 0) 
                                    printf("true\n");
                                else printf("false\n");
                            }    
                            else return EXIT_FAILURE;  
                    ...
                    if (strcmp("bijective", commands[command_count -1].command_string) == 0) {
                        if (command_arg_check(&commands[command_count -1], 1) == 0)
                            return EXIT_FAILURE;
                        else rel_biject(&rels_array[rel_index_check(commands[command_count - 1], rels_array, rel_count, 0)], uni_array);
                    }              
                }
...
void is_reflexive(Rel_t* rel_p, Universe_t* uni_p) {
	...
	// every element on the diagonal must be a "1"
	for (int i = 0; i < uni_p->member_count; i++) {
		if (getflag2d(i, i, rel2d.bytes, uni_p->member_count) == 0) {
...
void is_symmetric(Rel_t* rel_p, Universe_t* uni_p) {
    ...
    for (int x = 0; x < uni_p->member_count; x++) {
        for (int y = 0; y < uni_p->member_count; y++) {
            ...
            // for every a: a must be related to a
            if (a != b) {
...
void is_transitive(Rel_t* rel_p, Universe_t* uni_p) {
    ...
    for (int y = 0; y < uni_p->member_count; y++) {
        ...
        for (int x = 0; x < uni_p->member_count; x++) {
            ...
            // if aRb
            if (getflag2d(y, x, rel2d.bytes, uni_p->member_count) != 0) {
                
                for (int z = 0; z < uni_p->member_count; z++) {
                    
                    // if bRc 
                    bool bRc = getflag2d(x, z, rel2d.bytes, uni_p->member_count);
                    if (bRc != 0) {

                        bool aRc = getflag2d(y, z, rel2d.bytes, uni_p->member_count);

                        if (aRc) { // aRb and bRc and aRc
                            //printf("(%d, %d) and (%d, %d) AND (%d, %d)\n", y, x, x, z, y, z);
                        }
                        else { // aRb and bRc but not aRc
                            //printf("(%d, %d) and (%d, %d) BUT NO (%d, %d)\n", y, x, x, z, y, z);
...
void is_function(Rel_t* rel_p, Universe_t* uni_p) {
    ...
    // For each first component (y) is there only one second component (x)?
    for (int y = 0; y < uni_p->member_count; y++) {
        
        int rel_c = 0; // How many relations is this component in?
...
// BITFIELD MANIPULATION RELATED FUNCTIONS

// Turns a relation into a 2d bitfield. For example:
// universe: has 3 elemens
// relation: { (0,0), (0,1), (1,2) } will become:
// 110
// 001
// 000
// In this case, the bitfield will be made of 2 bytes and will look 
// like this in memory: [11000100][00000000], the last 7 bits are useless.
void rel_to_bitfield(Rel_t* rel_p, Universe_t* uni_p, Bitfield_t* field_p) {
...
// Returns what the parent byte of the bit on pos looks like, except besides pos 
// has been force-set to zero.
uint8_t getflag(int pos, uint8_t* flags) {
...
// If you imagine the bit field field_p as a 2d array with the given width, 
// what is the flag on the given coordinates?
uint8_t getflag2d(int y, int x, uint8_t* field_p, int bit_width) {
